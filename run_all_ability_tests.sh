...
if [ -z "$UNITY_PATH" ]; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        UNITY_PATH="/Applications/Unity/Hub/Editor/6000.4.5f1/Unity.app/Contents/MacOS/Unity"
    else
...
"$UNITY_PATH" -runTests -batchmode -projectPath "$PROJECT_PATH" -logFile "$LOG_FILE" -testResults "$TEST_RESULTS" -testPlatform EditMode -testFilter "Systems.AbilitySystem.Tests"
EXIT_CODE=$?
