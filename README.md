...
We use automated tests to verify code stability before integration. You can run these tests locally using the provided PowerShell (`.ps1`) or Bash (`.sh`) scripts.

### Prerequisites

1.  **Environment Variable:** Ensure the `UNITY_PATH` environment variable is set to the absolute path of your Unity executable.
    *   **Default Fallback (Windows only):** If not set, the PowerShell script will default to `"C:\Program Files\Unity\Hub\Editor\6000.4.5f1\Editor\Unity.exe"`. The Bash scripts require `UNITY_PATH` to be explicitly set.
    ...
    *   **To set it (macOS/Linux):**
        ...
        # Linux example
        export UNITY_PATH="/opt/Unity/Hub/Editor/6000.4.5f1/Editor/Unity"
        ```
...
*   **Common Failure:** If the script fails to start Unity, ensure your `UNITY_PATH` environment variable is explicitly set and points to an existing Unity `6000.4.5f1` installation.
