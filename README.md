...
1.  **Environment Variable:** Ensure the `UNITY_PATH` environment variable is set to the absolute path of your Unity executable.
    *   **Default Fallback (Windows):** If not set, the script will default to `"C:\Program Files\Unity\Hub\Editor\6000.4.5f1\Editor\Unity.exe"`.
    *   **Default Fallback (macOS/Linux):** If not set, the Bash script defaults to `/usr/bin/unity` (or `/Applications/Unity/Hub/Editor/6000.4.5f1/Unity.app/Contents/MacOS/Unity` if configured in your environment).
